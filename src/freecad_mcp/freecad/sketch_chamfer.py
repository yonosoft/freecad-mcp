"""Controlled chamfer operation for two intersecting normal line segments."""
...
from freecad_mcp.transaction_names import CHAMFER_SKETCH_GEOMETRY_TRANSACTION_NAME

_PublicOperation = Literal["chamfer"]
_MutationOperation = Literal["chamfer_geometry"]


@dataclass(frozen=True, slots=True)
class _ChamferPreflight:
...
def chamfer_sketch_geometry(
    ...
    """Trim two intersecting normal line segments and insert an equal-distance chamfer line."""
    operation: _MutationOperation = "chamfer_geometry"
    ...
    preflight = _preflight(document, sketch, snapshot, first_geometry_index, distance)

    if distance <= 0.0:
        raise SketchChamferUnsafeError(
            operation="chamfer",
            code="invalid_distance",
            reason="chamfer_distance_must_be_positive",
            first_geometry_index=first_geometry_index,
            details={"distance": distance},
        )

    caller_owned, owned, histories, active = _begin(
        document, snapshot, App, CHAMFER_SKETCH_GEOMETRY_TRANSACTION_NAME, operation
    )
    try:
        sketch.fillet(
        ...
        created_construction_arc_index = snapshot.sketch.geometry_count
        created_chamfer_line_index = snapshot.sketch.geometry_count + 1
        ...
        tangent_indices = _discover_chamfer_constraints(
            snapshot,
            first_geometry_index,
            preflight.position,
            removed_coincident_index,
        ...
        result = _build_result(
            first_geometry_index,
            second_geometry_index,
            created_construction_arc_index,
            created_chamfer_line_index,
            removed_coincident_index,
            tangent_indices,
            snapshot,
            inspected,
        ...
        _finish(
            ...
            caller_owned,
            owned,
            CHAMFER_SKETCH_GEOMETRY_TRANSACTION_NAME,
            operation,
        )
        return result
...
def _verify_chamfer(
    ...
    # Chamfer: +2 geos (construction arc + chamfer line)
    if inspected.geometry_count != snapshot.sketch.geometry_count + 2:
        raise _error("chamfer_geometry", "verification", "geometry_count_mismatch")

    # Chamfer: -1 coincident + 2 tangents + 2 coincidents = net +3
    if inspected.constraint_count != snapshot.sketch.constraint_count + 3:
        raise _error("chamfer_geometry", "verification", "constraint_count_mismatch")

    # verify source lines modified
    for idx in (first_index, second_index):
        actual = inspected.geometry[idx]
        if not isinstance(actual, SketchLineGeometry):
            raise _error("chamfer_geometry", "verification", "modified_line_type_mismatch")

    # verify construction arc at snapshot.geometry_count
    created_arc_index = snapshot.sketch.geometry_count
    arc = inspected.geometry[created_arc_index]
    if arc.to_dict().get("type", "") != "arc_of_circle":
        raise _error("chamfer_geometry", "verification", "created_geometry_not_arc")
    if not arc.construction:
        raise _error("chamfer_geometry", "verification", "construction_arc_expected")

    # verify chamfer line at snapshot.geometry_count + 1
    created_line_index = snapshot.sketch.geometry_count + 1
    chamfer_line = inspected.geometry[created_line_index]
    if not isinstance(chamfer_line, SketchLineGeometry):
        raise _error("chamfer_geometry", "verification", "created_chamfer_line_type_mismatch")
    ...
    for idx in range(snapshot.sketch.geometry_count):
        ...
        if before_geo.to_dict() != after_geo.to_dict():
            raise _error("chamfer_geometry", "verification", "unrelated_geometry_changed")
...
def _discover_chamfer_constraints(
    snapshot: Any,
    first_index: int,
    endpoint: int,
    removed_coincident_index: int,
) -> tuple[int, ...]:
    """Return indices of the four new constraints (2 tangents + 2 coincidents) after chamfer."""
    count = snapshot.sketch.constraint_count
    return (count - 1, count, count + 1, count + 2)
...
def _build_result(
    ...
    removed_coincident_index: int,
    chamfer_constraint_indices: tuple[int, ...],
    snapshot: Any,
    inspected: Any,
    summary: Any,
    caller_owned: bool,
    ...
    created_constraints = tuple(
        ...
        for idx in chamfer_constraint_indices
    )
    removed_constraints = (snapshot.sketch.constraints[removed_coincident_index],)
    return SketchChamferResult(
        first_geometry_index=first_index,
        second_geometry_index=second_index,
        created_construction_arc_index=created_construction_arc_index,
        created_chamfer_line_index=created_chamfer_line_index,
        removed_coincident_index=removed_coincident_index,
        created_tangent_indices=chamfer_constraint_indices,
        geometry_mappings=geometry_mappings,
        constraint_mappings=constraint_mappings,
        created_geometry=(
            SketchTopologyCreatedGeometry(
                index=created_construction_arc_index,
                geometry=inspected.geometry[created_construction_arc_index],
                reason="topology_result",
            ),
            SketchTopologyCreatedGeometry(
                index=created_chamfer_line_index,
                geometry=inspected.geometry[created_chamfer_line_index],
                reason="topology_result",
            ),
        ),
        removed_geometry=(),
        ...
        transaction_name=CHAMFER_SKETCH_GEOMETRY_TRANSACTION_NAME,
...
__all__ = ["chamfer_sketch_geometry"]
